...
func deleteEvent(entityId string, token *auth.Token) (*events.DeleteEvent, error) {

	deleteEvent := events.NewDeleteEvent(
		entityId,
		token.UID,
	)

	msg := gcloud.NewMessage(deleteEvent)

	publishError := gcloud.PublishEvent(context.Background(), *msg)
	if publishError != nil {
		log.Fatalf("Publish deleted events: %v", publishError)
	}
